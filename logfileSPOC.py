        ...
        with open(utils.LDA_FILE+utils.FILE_EXTENSION, 'w', encoding="utf8") as csvout:
            ...
            for array_line in rows:
                ...
                if user_id not in all_users or not is_consenting_student(user_id):  # this is a non-consenting student (although info is still public)
                    ...
                    file_out.writerow(cols + [days_after(datestamp, parent_id), str(dict_ld[int(parent_id)]), str([y[0] for y in utils.lecture_dates].index(int(parent_id))), len(comment), num_positive, num_negative, topic_name, str(is_help_request)] + all_users[user_id].to_const_string(utils.DELIMITER).split(utils.DELIMITER))
